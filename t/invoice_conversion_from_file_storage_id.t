...
$expected =556;
is(scalar(@rows) , $expected, "row count is $expected(" . scalar(@rows) . ")" );

my @row_length = uniq map{length($_)} @rows;
is(scalar(@row_length) , 1 , "all rows same number of characters");

is($row_length[0] , 80, "number of characters per row is 80");


my @ext = $controller->get_external_invoice_units;
ok(@ext, "got external invoices");
$expected =5;
is(scalar(@ext),  $expected, "number of invoices is $expected (". scalar(@ext) . ")" );
